...
static const wchar_t g_kClassNames[] = L"AddInNativeSql"; //"|OtherClass1|OtherClass2";
...
bool CAddInNative::SetPropVal(const long lPropNum, tVariant* varPropVal)
{
	...
	case ePropConnectionString:
		if (TV_VT(varPropVal) != VTYPE_PWSTR)
			return false;
		m_uiConnectionString = TV_WSTR(varPropVal);
		break;
	case ePropQuertyString:
		if (TV_VT(varPropVal) != VTYPE_PWSTR)
			return false;
	...
	tVariant* pvarRetValue, tVariant* paParams, const long lSizeArray)
{
	...
	case eMethExecute:
		...
		if (m_iMemory->AllocMemory((void**)&pvarRetValue->pstrVal, (wstr_.length() + 1) * sizeof(CHAR)))
		{
			memcpy(pvarRetValue->pstrVal, wstr_.c_str(), wstr_.length() * sizeof(CHAR));
			TV_VT(pvarRetValue) = VTYPE_PSTR;
			pvarRetValue->strLen = wstr_.length();
		}
		ret_ = true;
		break;
	}
